...
Shader "Custom/Unlit/Screen Space Decal"
{
    Properties
    {
        [Header(Basic)]
          ...
          //  TEXTURE2D(_CameraDepthTexture);
           // SAMPLER(sampler_CameraDepthTexture);
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                #else
                    float sceneRawDepth =  lerp(UNITY_NEAR_CLIP_VALUE, 1, SampleSceneDepth(screenSpaceUV));
                #endif
                //return float4(sceneRawDepth,sceneRawDepth,sceneRawDepth,1);
                ...
                col.r = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, uv + float2(+s, +s)).r;
                col.g = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, uv + float2(+s, -s)).g;
                col.b = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, uv + float2(-s, -s)).b;
